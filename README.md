...
AI 只生成可编辑初稿。用户在确认页点击“确认并保存”前，菜谱不会写入本地或云端状态。在新增或编辑菜品时，还可以先调整食材与用量，再点击“AI 生成”让云函数只重新生成操作步骤。

可选图片只作为菜谱配图，不会发送给 AI 模型。选择图片后小程序会先压缩再保存，菜谱列表和今日菜单使用懒加载，避免在首屏一次请求所有封面。今日页可直接分享当天菜单，接收者打开的是独立快照，不依赖分享者的 OpenID 数据。
